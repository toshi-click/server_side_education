...
## 不明な語句について
まずその語句をGoogleなどで検索して、それでも理解出来なかった場合はメンターに聞くようにしましょう
...
# ステップ1: 基本設定を行おう
このリポジトリを`git clone`したフォルダへコマンドプロンプト(Macの場合はターミナルで)で移動し`vagrant up`を行ってから始めてください。

また、終了する際には`vagrant halt`を行うようにしましょう。

※ VSCodeなどのエディタやIDEによってはプラグインでvagrantを便利に扱うことも出来ます。それは自由に利用してください。
...
- `app/htdocs`をドキュメントルートとしてアクセス出来るようにしてください。
- ブラウザで`http://vm.training.cs.test`へアクセスし接続できることを確認してください。
...
2000年代にはこのようなサーバー構築が一般的でしたが、2010年代以降は大部分を自動化することが出来るようになっています。  
...
`deploy/roles/sample`にrole作成で必要なディレクトリとファイルを配置しておきましたので、参考にしてください。
